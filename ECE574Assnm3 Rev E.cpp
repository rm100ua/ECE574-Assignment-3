...
//started from dat_newBranch Rev B.cpp
//modified get_sequence()
...
void alap_sequence(int &latency, int &error)
{
	int b = 0, d = 0, h = 0, k = 0;
	int cycle = 0, delay = 0;;
	string str = "", temp = "";
	...
	while (op_list[h].dp_outs[0] != "")
	{
		temp = op_list[h].dp_outs[0];
		found = str.find(temp);
		if (found != string::npos)
		{
			if (op_list[h].function == 1)
				op_list[h].ALAP_clock = delay - 1;
			if (op_list[h].function == 3)
				op_list[h].ALAP_clock = delay - 2;
			if ((op_list[h].function == 0) || (op_list[h].function == 2))
				op_list[h].ALAP_clock = delay;
		}
		h++;
	}
	...
	while (delay != 0)
	{
		while (op_list[b].dp_outs[0] != "")
		{
			if (op_list[b].ASAP_clock == cycle) 
				op_list[b].ALAP_clock = delay;
			b++;
		}
		delay--;
		cycle--;
...
void get_sequence(void)
{
	...
	string ready_inputs[40];				//array of inputs that are ready, allowing datapaths to be scheduled
	...
	for (p = 0; p < cir_list.inp_count; p++) {
	...
	while (op_list[dpc_idx].order) {			//check every dpc against the circuit inputs
		...
		{
			op_list[dpc_idx].seq_clock = i_clock;			//schedule the dpc
			cout << "Node  " << dpc_idx << ", Period:  " << op_list[dpc_idx].seq_clock << endl;
	...
	do {						//clock loop
		...
		do {										//dpc loop
			if (!op_list[dpc_idx].seq_clock)			//if this DPC is NOT scheduled, compare inputs to ready_inputs
			{
				...
				{
					op_list[dpc_idx].seq_clock = i_clock;			//schedule the DPC for this clock period
					cout << "Node  " << dpc_idx << ", Period:  " << op_list[dpc_idx].seq_clock << endl;
					p = 0;
					do {											//all dpc have at least 1 input
						ready_inputs[ready_ins_count++] = op_list[dpc_idx].dp_outs[p];
...
//creates the ASAP schedule.  similar to get_sequence above, different in that it has two dimensional array for
//ready inputs.  The first index being the clock cycle the ready input is available
void get_asap_sch2(void)
{
	int p = 0;								//counter used in for loop
	int n = 0;
	int k = 0;
	int inp_idx=0;								//index for inputs arrays
	int dpc_idx =0;							//index for data path components arrays
	int i_clock = 0;						//clock periods used for scheduling
	int count_dpc_scheduled = 0;			//count the Data Path Components (DPC) scheduled
	//string ready_inputs[40];				//array of inputs that are ready, allowing datapaths to be scheduled
	string ready_inputs[10][10];			//array of inputs that are ready, [clock period][input index]
	string dpc_inputs[12][10];				//DPC inputs [dpc_list.order][inp_index]
	string dpc_outputs[12][10];				//DPC inputs [dpc_list.order][inp_index]
	string temp_str = string();				//temporary string
	int beg_pos = 0;						//string index
	...
	for (p = 0; p < circuit_ins_count; p++) {
		//ready_inputs[p] = cir_list.ins[p];		//copy circuit inpputs
		ready_inputs[0][p] = cir_list.ins[p];;						//circuit inputs are available for first clock period, period 0
		cout << ready_inputs[0][p] << ", ";
	}
	cout << endl;
	...
	do {
		ready_ins_count++;				//count the number of ready inpuuts
	} while (ready_inputs[0][ready_ins_count] != "\0");
	cout << "ready inputs:  " << ready_ins_count << endl;
	...
	while (op_list[dpc_idx].order) {			//check every dpc against the circuit inputs
		if ((op_list[dpc_idx].dp_ins[0] == cir_list.ins[0]) || (op_list[dpc_idx].dp_ins[0] == cir_list.ins[1]) || (op_list[dpc_idx].dp_ins[0] == cir_list.ins[2])
			&& (op_list[dpc_idx].dp_ins[1] == cir_list.ins[0]) || (op_list[dpc_idx].dp_ins[1] == cir_list.ins[1]) || (op_list[dpc_idx].dp_ins[1] == cir_list.ins[2]))
		{
			op_list[dpc_idx].ASAP_clock = i_clock;			//schedule the dpc
			cout << "Node  " << dpc_idx << ", Period:  " << op_list[dpc_idx].ASAP_clock << endl;

			p = 0;											//add datapath outputs to ready_ouputs array
			do {											//all dpc have at least 1 input
				//ready_inputs[ready_ins_count++] = op_list[dpc_idx].dp_outs[p];
				switch (op_list[dpc_idx].function)		//allow for different latentcies
				{
					case 0:			//MUL
						ready_inputs[i_clock + 1][ready_ins_count++] = op_list[dpc_idx].dp_outs[p];
						break;
					case 1:			//DIV
						ready_inputs[i_clock + 2][ready_ins_count++] = op_list[dpc_idx].dp_outs[p];
						break;
					default:		// one clock cycle for all other operations
						ready_inputs[i_clock][ready_ins_count++] = op_list[dpc_idx].dp_outs[p];
						break;
				}
					p++;
			} while ((!op_list[dpc_idx].dp_outs[p].empty()) && (p < 3));
	...
	do {						//clock loop
		n = 0;
		do				//copy ready inputs to next cyle of ready inputs before incrementing the clock
		{
			ready_inputs[i_clock+1][n] = ready_inputs[i_clock][n];
			n++;
		} while (! ready_inputs[i_clock][n].empty() );
		i_clock++;				//advance to next clock period
		dpc_idx = 0;
		do {											//dpc loop
			if (!op_list[dpc_idx].ASAP_clock)			//if this DPC is NOT scheduled, compare inputs to ready_inputs
			{
				inp_idx = 0;
				do {								//dpc inputs loop
					rdy_idx = 0;
					inp_found = 0;
					do {					//ready inputs loop
						if (op_list[dpc_idx].dp_ins[inp_idx] == ready_inputs[i_clock][rdy_idx]) {
							inp_found = 1;
						}
						else {
							rdy_idx++;
					...
					//} while (!inp_found && !ready_inputs[rdy_idx].empty());		//until input is found or no more ready inputs
					} while (!inp_found && !ready_inputs[i_clock][rdy_idx].empty());		//until input is found or no more ready inputs
				inp_idx++;			//advance to next input
				} while (inp_found && !op_list[dpc_idx].dp_ins[inp_idx].empty());			//until all inputs are checked or an input is not found
				if (inp_found && !op_list[dpc_idx].dp_ins[inp_idx - 1].empty())
				{
...
int main(int argc, char *argv[])
{
	...
	if (myfile2.is_open()) // open output file check and write to output file
	{
		...
		while (m <= latency)
		{
			myfile2 << "Time " + std::to_string(m) + "\n";
			u = 0;
			while (op_list[u].dp_outs_str != "")
			{
				if (op_list[u].ASAP_clock == m)
				{					
