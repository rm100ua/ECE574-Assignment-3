...
void forceDSched(void)
{
	...
	while (sched_count < op_num)					//until all operations are scheduled
	{
		...
		op_list[low_node].FDSched = low_period;									//schedule this node
		op_list[low_node].low_f_period = low_period;							//record the low force period
...
int main(int argc, char *argv[])
{
	...
	if (myfile2.is_open()) // open output file check and write to output file
	{
		...
		myfile2 << "end else \n\tcase(state) \n";
		myfile2 << "\t\tIDLE : if (start == 1'b1) begin\n\t\t\tstate <= #1 S1;";
		myfile2 << "\n\t\t\tdone <= 0; \n";
		...
		while (m <= latency)
		{
			myfile2 << "\t\tS" << m << ": if (start == 1'b1) begin\n\t\t\tstate <= #1 S" << m + 1 << ";\n";
			u = 0;
