...
double MUL_op[40][40] = { 0.0 };     // Initialize 2D array to all 0
double ALU_op[40][40] = { 0.0 };     // Initialize 2D array to all 0
double Logic_op[40][40] = { 0.0 };   // Initialize 2D array to all 0
double DIV_op[40][40] = { 0.0 };     // Initialize 2D array to all 0

double MUL_dist[40];	//MUL distribution
double ALU_dist[40];
double Logic_dist[40];
double DIV_dist[40];
	...
	for (k = 0; k < s; k++)
	{
		...
		for (h = 0; h < 3; h++)
		{
			...
			while (op_list[b].dp_outs[0] != "" && (str != ""))
			{
				...
				if (temp == str)
				{
					op_list[b].ALAP_clock = op_list[node].ALAP_clock - (op_list[node].ASAP_clock - op_list[b].ASAP_clock);
					d = 0; p = 0;
					while (d < s)
					{
						if (vertex[d] == b)
...
void op_prob(void)
{
	...
	for (clk_count = 0; clk_count < l_cstrt; clk_count++) {
		...
		p = 0;
		do {
			MUL_dist[clk_count] = MUL_dist[clk_count] + MUL_op[p][clk_count];
			p++;
		} while (MUL_op[p][clk_count]);
	}
	//ALU
	for (clk_count = 0; clk_count < l_cstrt; clk_count++) {
		...
		p = 0;
		do {
			ALU_dist[clk_count] = ALU_dist[clk_count] + ALU_op[p][clk_count];
			p++;
		} while (ALU_op[p][clk_count]);
	}
	//Logic
	for (clk_count = 0; clk_count < l_cstrt; clk_count++) {
		...
		p = 0;
		do {
			Logic_dist[clk_count] = Logic_dist[clk_count] + Logic_op[p][clk_count];
			p++;
		} while (Logic_op[p][clk_count]);
	}
	//DIV
	for (clk_count = 0; clk_count < l_cstrt; clk_count++) {
		...
		p = 0;
		do {
			DIV_dist[clk_count] = DIV_dist[clk_count] + DIV_op[p][clk_count];
			p++;
		} while (DIV_op[p][clk_count]);
	}
}
